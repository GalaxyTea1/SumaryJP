...
const vocabularyManager = {
    ...
    checkReviewAnswer(correct) {
        ...
        apiManager.updateVocabulary(this.currentReviewVocab).then(() => {
    ...
    updateStatistics() {
        let totalWords = 0, mastered = 0, learning = 0, notLearned = 0;
            ...
            Object.values(level).forEach(lesson => {
                lesson.forEach(word => {
                    ...
                    switch(word.status) {
                        ...
                        case 'in-progress': learning++; break;
    ...
    drawProgressChart(mastered, learning, notLearned) {
        ...
        this.progressChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ data: [mastered, learning, notLearned], backgroundColor: ['#4CAF50', '#FFC107', '#F44336'] }]
            },
            options: {
                responsive: true,
    ...
    submitAnswer() {
        ...
        this.testAnswers.push({ word: currentWord, userAnswer: answer, correct: isCorrect });
    ...
    finishTest() {
        ...
        document.getElementById('time-taken').textContent = `${Math.floor(timeTaken / 60).toString().padStart(2, '0')}:${(timeTaken % 60).toString().padStart(2, '0')}`;
